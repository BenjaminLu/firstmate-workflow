...
# no library beside this script - every fixture from before projects existed -
# still depends on nothing and polls its own checkout, as it did then.
projects=''; default=''
if [ -f "$REPO/config.yaml" ] && [ -r "$HERE/fm-config.sh" ]; then
