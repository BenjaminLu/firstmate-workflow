...
# Focused caller fixture: exercises fm-run's decide handoff without git/gh.
# Authored --details is T-034 scope; until that decide lands, assert the
# title-only path that this checkout actually ships.
caller="$(mktemp -d)"
...
cp "$ROOT/bin/fm-herdr.py" "$caller/bin/" 2>/dev/null || true
...
if grep -q -- '--details)' "$caller/bin/fm-decide.sh"; then
  ...
  valid="$(PATH="$caller/bin:$PATH" bash "$caller/bin/fm-run.sh" once --repo "$caller" 2>&1)"
  assert_contains "$valid" 'asking the captain (D-991)' 'title-only decide still creates an announced card'
  assert_ok "test -f '$caller/state/pending/D-991.json'" 'title-only pending card is on disk'
fi
