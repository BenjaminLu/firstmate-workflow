...
3. In a user-managed Herdr session (`HERDR_ENV=1`), check `herdr` availability
   ...
   and adapter path in a dedicated unfocused tab with one owned root pane. Use
   `herdr tab create --workspace <caller-workspace> --cwd <tree>
   --label <canonical-actor> --no-focus`; never split or reuse the captain view.
   Record caller tab/pane and observed UI focus before and after creation;
   changed or unknown focus refuses launch without taking focus back. Record
   owned workspace/tab/pane, task, role and session identities. Reuse existing agents. An
   ...
   The shipped adapters now provide this transport by default; no ignored wrapper
   or global hook is needed. See the [managed session contract](../../design/design.md#managed-session-defaults).
...
- `bin/fm-dispatch.sh --repo <root> --dry-run` previews ready tasks. Actual dispatch
  ...
  background orchestration launches the actual adapter CLI in a dedicated unfocused tab/root pane
  when `HERDR_ENV=1`; `FM_TRANSPORT=direct` explicitly opts out.
